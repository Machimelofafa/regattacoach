...
def create_demo_data():
    """Crée des données de démonstration pour la zone de course"""
    ...
    # Bateau 1 - MAORI III
    positions1 = []
    base_lat, base_lon = 50.7, -1.2  # Sud de l'Angleterre
    for i in range(20):
        positions1.append({
            "lat": base_lat + i * 0.01,
            "lon": base_lon - i * 0.02,
...
def extract_boats_by_class(race_setup, classes, latest_positions=None):
    ...
    # Si aucun bateau n'a été trouvé dans RaceSetup mais que nous avons des données LatestPositions,
    # utiliser ces données pour créer une liste de bateaux
    if not boats_by_class["Tous les bateaux"] and latest_positions and "positions" in latest_positions:
...
def fetch_boat_positions(race_id, boat_id):
    ...
    """
    url = f"https://cf.yb.tl/JSON/{race_id}/boat/{boat_id}"
    boat_data = safe_get_json(url)
    
    if not boat_data or "positions" not in boat_data:
        return []
...
def main():
            ...
            if race_id.strip():
                with st.spinner("Téléchargement des données de course..."):
                    ...
                    if st.session_state.race_data.get("RaceSetup"):
                        ...
                        st.success("Données de course téléchargées avec succès")
                    else:
    ...
    # Contenu principal
    if "boats_by_class" in st.session_state and "selected_boats" in st.session_state and st.session_state.selected_boats:
        ...
        if "race_data" in st.session_state and "boats_data" in st.session_state:
            ...
            # Si aucune donnée n'a été récupérée, utiliser des données de démonstration
            if not boats_data:
                st.warning("Aucune donnée de position valide n'a pu être récupérée. Utilisation de données de démonstration.")
                boats_data = create_demo_data()
    ...
    elif "boats_by_class" not in st.session_state:
